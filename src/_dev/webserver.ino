/*
  webserver.cpp - webserver for ESPKNXMQTT

  based on source Sonoff-Tasmota by Theo Arends (https://github.com/arendst/Sonoff-Tasmota)
...
void HandleBackupConfiguration()
{
    ...
    { // https://github.com/esp8266/Arduino/issues/3218
...
#ifdef USE_RF_FLASH
        case 10:
            strncpy_P(error, PSTR(D_UPLOAD_ERR_10), sizeof(error));
            break;
        case 11:
            strncpy_P(error, PSTR(D_UPLOAD_ERR_11), sizeof(error));
            break;
        case 12:
            strncpy_P(error, PSTR(D_UPLOAD_ERR_12), sizeof(error));
            break;
        case 13:
            strncpy_P(error, PSTR(D_UPLOAD_ERR_13), sizeof(error));
            break;
#endif
        default:
...
void HandleUploadLoop()
{
    ...
    if (upload_error)
    {
        if (UPL_TASMOTA == upload_file_type)
        {
            Update.end();
        }
        return;
            ...
            if (web_log_index != curridx)
            {
                ...
                do
                {
                    ...
                    if (len)
                    {
                        ...
                        if (JSON)
                        { // Is it a JSON message (and not only [15:26:08 MQT: stat/wemos5/POWER = O])
                            if (message.length() > 1)
                            {
                                message += F(",");
                            }
                    ...
                    counter++;
                    if (!counter)
                        counter++; // Skip 0 as it is not allowed
